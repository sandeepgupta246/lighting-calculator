import streamlit as st
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
from io import BytesIO

# ===================== SAFE LUXPY IMPORT =====================
LUXPY_AVAILABLE = False
lx = None
try:
    import luxpy as lx
    LUXPY_AVAILABLE = True
    st.sidebar.success("✅ luxpy loaded — IES support enabled")
except Exception as e:
    LUXPY_AVAILABLE = False
    st.sidebar.warning(f"⚠️ luxpy could not load (common with NumPy 2.x). IES upload disabled. Error: {str(e)[:80]}...")
    st.sidebar.info("Fix: Run 'pip install \"numpy<2.0\"' in a fresh venv and restart.")

st.title("🪔 Professional Lighting Calculator (Safe Version)")

st.sidebar.header("Room Details")
length_m = st.sidebar.slider("Length (m)", 1.0, 20.0, 3.05)
width_m = st.sidebar.slider("Width (m)", 1.0, 20.0, 3.05)
ceiling_height_m = st.sidebar.slider("Ceiling Height (m)", 2.0, 5.0, 3.0)
workplane_height_m = st.sidebar.slider("Workplane Height (m)", 0.0, 1.5, 0.8)

room_type = st.sidebar.selectbox("Room Purpose", 
    ["Living Room / General", "Bedroom (Cozy)", "Kitchen / Office", "Reading / Task"])

ceiling_ref = st.sidebar.slider("Ceiling Reflectance", 0.5, 0.9, 0.8)
wall_ref = st.sidebar.slider("Wall Reflectance", 0.2, 0.8, 0.5)
floor_ref = st.sidebar.slider("Floor Reflectance", 0.1, 0.4, 0.2)

lux_dict = {"Living Room / General": 300, "Bedroom (Cozy)": 200, "Kitchen / Office": 500, "Reading / Task": 750}
desired_lux = st.sidebar.slider("Target Lux", 100, 1000, lux_dict[room_type])

# ===================== CALCULATIONS =====================
area_m2 = length_m * width_m
h_rc = ceiling_height_m - workplane_height_m
rcr = (5 * h_rc * (length_m + width_m)) / area_m2 if area_m2 > 0 else 0
llf = 0.80

def get_manufacturer_cu(fixture_type, rcr_val, ceiling_r, wall_r):
    base_cu_table = {
        "Recessed Downlight": [0.82, 0.76, 0.69, 0.63, 0.57, 0.52],
        "LED Panel": [0.85, 0.80, 0.74, 0.68, 0.63, 0.58],
        "Surface Batten": [0.78, 0.72, 0.66, 0.60, 0.55, 0.50],
        "Pendant": [0.72, 0.67, 0.61, 0.56, 0.51, 0.47],
        "Wall Sconce": [0.65, 0.60, 0.55, 0.50, 0.46, 0.42]
    }
    fixture_key = next((k for k in base_cu_table if k.lower() in fixture_type.lower()), "Recessed Downlight")
    base_cu = np.interp(rcr_val, [1,2,3,4,5,6], base_cu_table[fixture_key])
    reflectance_factor = (ceiling_r * 0.6 + wall_r * 0.3) / (0.8*0.6 + 0.5*0.3)
    return max(0.40, min(0.88, base_cu * reflectance_factor))

# Fixture database (realistic Havells/Philips style)
fixtures = pd.DataFrame({
    "Type": ["Recessed Downlight", "LED Panel (2x2 ft)", "Surface Batten", "Pendant Light", "Wall Sconce"],
    "Model Example": ["Havells Endura DL / Philips CoreLine", "Havells Venus Neo", "Havells Destello", "Havells Saber", "Havells Spirit"],
    "Lumens": [1200, 3600, 2000, 2500, 850],
    "Wattage": [12, 36, 20, 25, 10],
    "Price ₹": [480, 1350, 380, 920, 650]
})

# ===================== IES UPLOAD (luxpy) =====================
ies_data = None
if LUXPY_AVAILABLE:
    st.subheader("📤 Upload Real Manufacturer IES File")
    uploaded_ies = st.file_uploader("Upload .ies file", type=["ies"])
    if uploaded_ies:
        try:
            ies_bytes = uploaded_ies.read()
            with open("temp.ies", "wb") as f:
                f.write(ies_bytes)
            
            lamp_data = lx.io.read_lamp_data("temp.ies", verbosity=0)
            ies_data = lamp_data
            
            st.success("✅ IES file loaded!")
            col1, col2 = st.columns(2)
            with col1:
                st.write("**Flux**:", round(lamp_data.get('flux', 0), 0), "lm")
            with col2:
                st.write("**CCT**:", lamp_data.get('cct', 'N/A'))
        except Exception as e:
            st.error(f"IES read error: {str(e)}")

# ===================== RESULTS =====================
st.subheader("📊 Results (Lumen Method + Real CU)")
st.write(f"**Room Area**: {area_m2:.1f} m²")
st.write(f"**Room Cavity Ratio (RCR)**: {rcr:.2f}")

# Use IES lumens if available, else fallback
use_lumens = ies_data.get('flux', 1200) if ies_data else 1200

remaining_lumens_needed = area_m2 * desired_lux * llf
qty = max(1, int(np.ceil(remaining_lumens_needed / use_lumens)))

st.write(f"**Total Maintained Lumens Needed**: {remaining_lumens_needed:.0f} lm")
st.success(f"**Recommended Quantity**: **{qty}** fixtures (using {'IES data' if ies_data else 'default lumens'})")

# Optional simple recommendation table
st.subheader("💡 Sample Fixture Options")
recs = []
for idx, row in fixtures.iterrows():
    cu = get_manufacturer_cu(row["Type"], rcr, ceiling_ref, wall_ref)
    delivered = row["Lumens"] * cu
    cost = row["Price ₹"] 
    recs.append({
        "Fixture": row["Type"],
        "Qty Suggestion": max(1, int(np.ceil(remaining_lumens_needed / row["Lumens"]))),
        "CU": f"{cu:.3f}",
        "Est. Total Cost ₹": int(max(1, int(np.ceil(remaining_lumens_needed / row["Lumens"]))) * cost)
    })
st.dataframe(pd.DataFrame(recs), use_container_width=True)

st.caption("✅ App is running safely. If luxpy failed, basic calculation still works perfectly. For full IES support, fix NumPy version as instructed above.")

# Placement tips
st.subheader("📍 Placement Tips")
st.write("Use even grid with Spacing-to-Height Ratio (SHR) ≈ 1.2–1.5")
st.caption("For highest accuracy, import the same IES file into free DIALux evo software.")